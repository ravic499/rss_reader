...
class RSSReaderApp(MDApp):
    ...
    def load_feeds(self):
        all_articles = fetch_all_feeds(self.feeds)  # returns dict keyed by url
        print("Fetched all_articles:", all_articles)  # Debug print
        # Update self.feeds with articles fetched
        for feed in self.feeds:
            url = feed.get('url')
            if url in all_articles:
                feed['articles'] = all_articles[url]['articles']
                feed['category'] = all_articles[url].get('category', feed.get('category', ''))
            else:
                feed['articles'] = []
        self.all_articles = all_articles
        print("Feeds after merging articles:", self.feeds)  # Debug print
        return self.feeds
